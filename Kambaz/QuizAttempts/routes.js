...
export default function QuizAttemptsRoutes(app) {
  ...
  // 取得目前登入學生在某 quiz 的所有作答
  app.get("/api/quizzes/:quizId/attempts/me/all", async (req, res) => {
    const { quizId } = req.params;
    const currentUserId = req.session?.currentUser?._id;
    if (!currentUserId) {
      return res.status(401).send({ message: "Not logged in" });
    }
    const attempts = await dao.findAttemptsForQuizAndUser(quizId, currentUserId);
    res.send(attempts);
  });
  ...
  app.get("/api/quizzes/:quizId/attempts/me", async (req, res) => {
    ...
    const latest = await dao.findLatestAttemptForQuizAndUser(quizId, currentUserId);
    res.send(latest);
  });
